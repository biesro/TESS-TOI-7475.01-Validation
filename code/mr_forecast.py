...
def Rpost2M(radius, unit='Earth', grid_size = 1e3, classify = 'No'):
    ...
    """
    # Fix per a NumPy modern: grid_size ha de ser sencer
    ...
    if unit == 'Earth':
        ...
        # Corregit amb parèntesis per a Python 3
        print("Input unit must be 'Earth' or 'Jupiter'. Using 'Earth' as default.")
...
def Rstat2M(mean, std, unit='Earth', sample_size=1e3, grid_size=1e3, classify = 'No'):   
    ...
    """
    # Fix per a NumPy modern: sample_size i grid_size han de ser sencers
    ...
    onesigma = 34.1
    m_up = np.percentile(mass, 50.+onesigma, method='nearest') # 'interpolation' ha canviat a 'method' en versions noves
    m_down = np.percentile(mass, 50.-onesigma, method='nearest')
